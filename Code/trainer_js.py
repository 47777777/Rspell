...
class Trainer:
    ...
    def train(
        ...
        # tr_loss is a tensor to avoid synchronization of TPUs through .item()tr_loss是一个张量，以避免通过.item()实现TPU的同步。
        tr_loss = torch.tensor(0.0).to(args.device)
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses_total_loss_scalar在每次调用tr_loss的.item()时都会更新，并存储所有损失的总和。
        ...
        # Skip the first epochs_trained epochs to get the random state of the dataloader at the right point.跳过第一个 epochs_trained epochs，以便在正确的点上获得dataloader的随机状态。
        if not args.ignore_data_skip:
        ...
        for i in range(len(inputs)):
            for k, v in inputs[i].items():
                if isinstance(v, torch.Tensor):
                    inputs[i][k] = v.to(self.args.device)

            if self.args.past_index >= 0 and self._past is not None:
                inputs[i]["mems"] = self._past
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        outputs1= model(**inputs[1])

        # model1 和tgt比较!!!
        w=[]
        for i in range(len(inputs[2]['input_ids'])):
            num_in=0
            num_other=0
            for j in range(len(inputs[2]['input_ids'][i])):
                if inputs[2]['input_ids'][i][j] in inputs[3]['input_ids'][i] and inputs[2]['input_ids'][i][j]!=101 and inputs[2]['input_ids'][i][j]!=102 and inputs[2]['input_ids'][i][j]!=0 and inputs[2]['input_ids'][i][j]!=117:
                    num_in+=1
            if num_in==0:
                w=0
            else:
        ...
        if labels is not None:
            ...
            loss1 = self.label_smoother(outputs1, labels)
    ...
    def do_test(self):
        ...
        metric = _compute_metrics(results, print_result=False)
        dacc, dpre, drec, df1, cacc, cpre, crec, cf1 = metric
        return {"dacc": round(dacc, 4), "dpre": round(dpre, 4), "drec": round(drec, 4), "df1": round(df1, 4),
        ...
        if self.compute_metrics is not None and evaluation_results is not None:
            ...
            if len(src_sents) != 0 and len(gold_sents) != 0 and len(pre_sents) != 0:
                ...
                for i in range(len(src_sents)):
                    ...
                    src_sents[i] = src_sents[i][0:len(gold_sents[i])]
                    sent = src_sents[i]
    ...
    def prediction_step(
        ...
        has_labels = all(inputs[0].get(k) is not None for k in self.label_names)
        inputs = self._prepare_inputs(inputs)
        ...
        if has_labels:
            labels = nested_detach(tuple(inputs[0].get(name) for name in self.label_names))
            if len(labels) == 1:
                labels = labels[0]
        else:
            labels = None
    ...
    def prediction_loop(
        ...
        for step, inputs in enumerate(dataloader):
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            # loss= self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            if loss is not None:
...
def _compute_metrics(results: Union[List, numpy.ndarray], helper: EvalHelper,):
    dacc, dpre, drec, df1, cacc, cpre, crec, cf1 = helper.compute(results, print_result=False)
    return {
...
def convert_ids_to_tokens(src, gold, preds, config, unique_tag, offset=1):
    ...
    for src_sent, gold_ids, pre_ids in zip(src, gold, preds):
        ...
        for i in gold_ids:
            if i!=-100:
                sum+=1
        src_sent = src_sent[0:sum]
        gold_ids = gold_ids[0:sum]
