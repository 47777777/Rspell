...
def tagger(
    ...
    with torch.no_grad():
        ...
        for i in trange(len(test_encodings.data["input_ids"]), desc="Generate results"):
            ...
            pre_id, pre_sent, res , candidate= customize(logits, texts[i], vocab_processor, model.config, labels,
                                              min_logit=min_logit, vocab_feature=test_word_features[i], weight=weight,
                                              ASM=ASM, RSM=RSM, b_search=False)

            
            temppre=pre_sent[1:-1]
            temptext=texts[i]
            kk=[]
            assert len(temppre)==len(temptext)
...
def customize(logits, src, processor, config, labels, vocab_feature=None,
    ...
    # def ppl(text,model,tokenizer,device,src):
    #     with torch.no_grad():
    #         # model = BertForMaskedLM.from_pretrained('/data1/ssq/experiment/ECSpell-main/Transformers/chinese-roberta-wwm-ext')
    #         model.to(device)
    #         model.eval()
    #         # Load pre-trained model tokenizer (vocabulary)
    #         # tokenizer = BertTokenizer.from_pretrained('/data1/ssq/experiment/ECSpell-main/Transformers/chinese-roberta-wwm-ext')
    ...
    #             mask_input=Variable(mask_input).to(device)
    ...
    if len(res_sents) != 1:
        ...
        # small=float('inf')
        # topscores=heapq.nlargest(5, range(len(scores)), scores.__getitem__)
        # for k in topscores:
        #     ppl_score=ppl(res_sents[k],model1,tokenizer1,device,src)
        #     if small>ppl_score:
        #         small=ppl_score
        #         largest_prob=k
    ...
    return res_sent_id, [0] + res_sent + [0], candidate_path_num,candidate
